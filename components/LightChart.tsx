...
export default function LightChart({ symbol, height = 520 }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const chart: ChartApi = createChart(el, {
        ...
        crosshair: { mode: 1, vertLine: { color: "rgba(255,255,255,0.12)" }, horzLine: { color: "rgba(255,255,255,0.12)" } },
      ...
      const vol = chart.addHistogramSeries({ priceFormat: { type: "volume" }, priceScaleId: "", base: 0 });
      ...
      chart.subscribeCrosshairMove((param:any) => {
        const t = typeof param.time === "number" ? param.time : (param.time?.timestamp ?? undefined);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const ok = res.ok; const data = ok ? await res.json() : null;
        ...
        if (cancelled) return;
        const map = new Map<number, any>(); candles.forEach(c => map.set(c.time, c)); dataMapRef.current = map;
        ...
        if (!cancelled) {
          const m = genMock();
          const map = new Map<number, any>(); m.candles.forEach(c => map.set(c.time, c)); dataMapRef.current = map;
  ...
  return (
    <div ref={wrapRef} className="rounded-2xl overflow-hidden border border-white/10 relative shadow-[0_8px_30px_rgba(0,0,0,0.25)]" style={{ height, width: "100%" }}>
      <div className="absolute inset-0 pointer-events-none" style={{background:"radial-gradient(1200px 400px at 20% -20%, rgba(255,255,255,0.06), transparent)"}}/>
      ...
      {hover && !loading && (
        <div className="absolute top-2 left-2 text-[11px] bg-black/60 border border-white/10 rounded px-2 py-1.5">
          <div>O:{hover.open?.toFixed?.(2)}  H:{hover.high?.toFixed?.(2)}  L:{hover.low?.toFixed?.(2)}  C:{hover.close?.toFixed?.(2)}</div>
        </div>
      )}
