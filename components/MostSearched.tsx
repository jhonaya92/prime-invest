    ...
    <div className="card">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        {items.map((it) => {
          ...
          return (
            <a
              key={it.sym}
              href={`/ativos/${encodeURIComponent(it.sym)}`}
              className="block rounded-xl bg-white/5 border border-white/10 p-3 hover:bg-white/10 transition"
            >
              ...
              <div className="mt-1 text-sm">
                {it.price !== undefined ? (
                  <>
                    ...
                    <span
                      className={`ml-2 ${up ? "text-green-400" : "text-red-400"}`}
                    >
                      {up ? "+" : ""}
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        {["BBDC4", "BBAS3", "PRIO3"].map((c) => (
          <a
            key={c}
            href={`/ativos/${encodeURIComponent("BMFBOVESPA:" + c)}`}
